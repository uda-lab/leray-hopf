...
from `exists_lerayHopf_torus3`.  It contains NO new mathematics: the finite-dimensional
...
of the torus solver chain here via `TorusModeTail → TorusModeCompactness → SolutionInterfaces`.
...
(relocated from `SolutionInterfaces.lean`) so it can call the proved limit-passage theorems in
...
`galerkin_ode_solution` axiom drops EXACTLY that axiom from `exists_lerayHopf_torus3`'s
...
- `build_galerkin_package_of_galSeq` — core assembly A2 → proved limit passage (relocated from `SolutionInterfaces.lean`)
- `build_galerkin_package_of_torus`  — full package via the axiom-free builder
- `exists_lerayHopf_torus3` — main existence theorem (relocated from `TorusConvectionForm`)
-/
...
/-- **Assembly (axiom-free core, relocated from `SolutionInterfaces.lean`).**
...
carries NO dependency on A1.  Relocated here from `SolutionInterfaces.lean` so that it can call
the proved theorems `torus_galerkin_limit_passage_of_energyClass` and
`torus_energyClass_of_aubinLions` (which are downstream of `SolutionInterfaces`; keeping this def
...
/-- **Main existence theorem (capstone) — rerouted through the proved solver (issue #24).**
