...
- **B7 `convFormH1_bound_Schwartz`** [must-prove — CODEX REVIEW on exponent arithmetic]: for Schwartz `w` and `u,v ∈ H1_sigma`: `|convFormH1 u v w| ≤ C_w * ‖(u : L2VF_R3)‖ * ‖(v : L2VF_R3)‖` where `C_w` depends on Schwartz seminorms of `w`. Proof: GNS (u,v ∈ L⁶ via A3) + Hölder `L⁶·L²·L^∞ ≤ L¹` for Schwartz `w` (Schwartz ⊂ L^∞ ∩ L²). The KEY: the FINAL bound is in L²-NORMS of u,v (not H¹-norms), enabling BLT extension to all of L².
