...
| R3 | **Whole-space ℝ³ Leray–Hopf existence** (`exists_lerayHopf_r3_axiomatic`) — the real target (Leray 1934) | **DONE** — proved modulo exactly 6 Codex-approved axioms; ℝ³ spatial+regularity layer built axiom-free; abstract layer reused unmodified |
...
unbounded domain. `R3NSForms.b_self_zero` is a proved lemma. `#print axioms exists_lerayHopf_r3_axiomatic` →
exactly these 6 + `propext`/`Classical.choice`/`Quot.sound` (no `sorryAx`). Renamed from
`exists_lerayHopf_r3` in Wave-0 (Issue #1 item 3).
