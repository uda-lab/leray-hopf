...
| B | **LOCAL Rellich via Fréchet–Kolmogorov** (`LerayHopf/R3/RellichBall.lean`) — thins P3's `LocalRellichInput` toward the standard FK criterion | **PARTIAL** (honest) — `localRellichInput_of_frechetKolmogorov (FK : FrechetKolmogorovInput) : LocalRellichInput` is fully ASSEMBLED; `translate_L2VF` (T0a), the sqrt bound (T0c), the uniform family bound (T1b), and the T2 reduction are proved axiom-free. **OPEN** (sole `sorry`, sorryAx isolated to it): `normSq_translate_sub_le_viscousFormSq` (T0b), the Plancherel translation-modulation estimate `‖τ_h w−w‖² ≤ ‖h‖²·viscousFormSq_R3 1 w` — blocked because mathlib lacks the Lp-level Fourier modulation identity `𝓕(τ_h f)=phase·𝓕 f` (needs a bounded phase-multiplier on L²C + Schwartz modulation + density extension; NOT impossible, just unbuilt). Isolated hypothesis: `FrechetKolmogorovInput` (standard FK precompactness criterion). `exists_lerayHopf_r3` unaffected |
