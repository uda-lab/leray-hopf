...
- **Finite time horizon, not global-in-time.** `T : ℝ` with `hT : 0 < T` is an input to
  the capstones: for each such `T` there is a solution on `[0, T]`. This is not a claim
  of a single solution simultaneously valid on `[0, ∞)`.
