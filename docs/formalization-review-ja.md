...
弱 NS 方程式 (`WeakFormNS`)、`[0,T]` 上のエネルギー不等式、初期トレース、
エネルギー類 `u∈L²(0,T;H¹_σ)` の四つを**実際の証明として**フィールドに持つ。
...
（`∀ᵐ t, memH1VF (u t)` ＋ `IntervalIntegrable (viscousFormSq ν ∘ u)`）。これで `u∈L²(0,T;H¹_σ)` を強制。
（Codex T³ v5 / ℝ³ にも継承。）
