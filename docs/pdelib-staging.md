...
|---|---|---|---|
| `LerayHopf/Bochner/TimeSobolev.lean` | Lions–Magenes vector-valued time-Sobolev apparatus: `IsWeakTimeDeriv`, the abstract Gelfand-triple embedding `H ↪ V'`, `W1pTime`. | verbatim-ready (after issue #113 PR-4 commit 3, which deleted the dead `GelfandTriple.ιCLM` shim) | None; sorry-free since issue #147 (the former `w1pTime_continuous_in_H` `ALLOW_SORRY` was relocated to `LerayHopf/Bochner/TimeSobolevExperimental.lean:71`, a declared months-class Lions–Magenes good-representative residual, there restricted to `p = q = 2` after issue #158 found the prior generic-`p,q` statement FALSE — not merely relocated, corrected). |
| `LerayHopf/Bochner/TimeSobolevAC.lean` | The absolutely-continuous-in-time good representative built on top of `TimeSobolev.lean`. | verbatim-ready | Imports only `TimeSobolev.lean` and mathlib. One pre-existing `ALLOW_SORRY` (`LerayHopf/Bochner/TimeSobolevAC.lean:322` — the Bochner–Fubini distributional FTC for the interval primitive, isolated single residual of R1); the statement is intact and the gap is tracked, not hidden. |
